...
Value getworkaux(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Groupcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Groupcoin is downloading blocks...");
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 51332));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    SSLIOStreamDevice d(sslStream, fUseSSL);
    iostreams::stream<SSLIOStreamDevice> stream(d);
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "51332")))
        throw runtime_error("couldn't connect to server");
#else
    if (fUseSSL)
        throw runtime_error("-rpcssl=1, but bitcoin compiled without full openssl libraries.");

    ip::tcp::iostream stream(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "51332"));
    if (stream.fail())
        throw runtime_error("couldn't connect to server");
#endif
