...
{
    // Windows: C:\Documents and Settings\username\Application Data\Groupcoin
    // Mac: ~/Library/Application Support/Groupcoin
    // Unix: ~/.groupcoin
#ifdef __WXMSW__
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) + "\\Groupcoin";
#else
...
string GetConfigFile()
{
    ...
    fs::path pathConfig(GetArg("-conf", "groupcoin.conf"));
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "groupcoind.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
